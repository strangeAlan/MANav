...
cleanup() {
  if [[ "${LINGBOT_STARTED_BY_RUN}" == "1" ]]; then
    echo "[run_iin_sparse_lingbot] stopping LingBot depth server started by this run"
...
if ! curl -fsS --max-time 2 "http://127.0.0.1:${LINGBOT_PORT}/health" >/dev/null 2>&1; then
  echo "[run_iin_sparse_lingbot] LingBot depth server is not healthy; starting it first"
  LINGBOT_STARTED_BY_RUN=1
  RUN_IN_BACKGROUND=1 LINGBOT_PORT="${LINGBOT_PORT}" script/start_lingbot_depth_server.sh
fi

CONFIG_FILE="${CONFIG_FILE:-configs/config_iin_sparse_lingbot_local_qwen.yaml}" \
EXPERIMENT_ID="${EXPERIMENT_ID:-iin_sparse_lingbot_local_qwen}" \
./script/run_iin.sh
