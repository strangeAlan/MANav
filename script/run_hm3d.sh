...
CONFIG_FILE="${CONFIG_FILE:-configs/config_objectnav_hm3d_local_qwen.yaml}"
...
if ! curl -fsS --max-time 2 "http://127.0.0.1:18080/health" >/dev/null 2>&1; then
  echo "[run_hm3d] local Qwen is not healthy; starting it first"
  RUN_IN_BACKGROUND=1 script/start_local_vlm.sh
fi

RUN_CONFIG="${CONFIG_FILE}"
if [[ -n "${NUM_EVAL_EPISODES}" || "${EPISODE_ID}" != "-1" ]]; then
  RUN_CONFIG="/tmp/unigoal_objectnav_hm3d_${USER:-user}_$$.yaml"
...
echo "[run_hm3d] ObjectNav-HM3D config=${RUN_CONFIG} episode=${EPISODE_ID} gpu=${NAV_GPU}"
